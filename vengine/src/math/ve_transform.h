...
namespace VE {
struct Transform {
   __forceinline Transform() noexcept
      : position(0, 0, 0)
      , rotation(0, 0, 0, 1)
      , scale(1, 1, 1)
   {}
   ...
   __forceinline Transform getInversed() const noexcept
   {
      ...
      inv.scale.x = fabsf(scale.x) < VE::EPSILON ? 0.0f : 1.0f / scale.x;
      inv.scale.y = fabsf(scale.y) < VE::EPSILON ? 0.0f : 1.0f / scale.y;
      inv.scale.z = fabsf(scale.z) < VE::EPSILON ? 0.0f : 1.0f / scale.z;
