...
namespace VE {
struct Vector3 {
   __forceinline Vector3(float fillValue = 0.0f) noexcept
      : x(fillValue)
      , y(fillValue)
      , z(fillValue){};

   __forceinline Vector3(float x_, float y_, float z_ = 0.0f) noexcept
      : x(x_)
      , y(y_)
      , z(z_){};

   __forceinline Vector3(const float* fv) noexcept
      : x(fv[0])
      , y(fv[1])
      , z(fv[2]){};
   ...
   {
      return (*this - other).lenSqrt() < VE::EPSILON;
   }
   ...
   __forceinline static float angle(const Vector3& a, const Vector3& b) noexcept
   {
      ...
      float bLenSqrt = b.lenSqrt();
      if (aLenSqrt < VE::EPSILON || bLenSqrt < VE::EPSILON) {
         return 0.0f;
      }
...
struct TVector4 {
   ...
   TVector4(T x_, T y_, T z_, T w_ = 1.0f)
      : x(x_)
      , y(y_)
      , z(z_)
      , w(w_)
